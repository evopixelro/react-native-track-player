...
Use this format to keep changes easy to review. This fork prepares release notes
and publishes releases manually; commit messages do not trigger a release.

Run `npm run check` before submitting changes. For an interactive commit prompt,
use `npm run commit`. The original Husky hook is optional: enable it with
`npm run hooks:install` after installing development dependencies.
...
Use `feat`, `fix` or `perf` for user-facing changes to include in release notes.
Always document a [BREAKING CHANGE](#footer), regardless of its commit type.
