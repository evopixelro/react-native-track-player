...
import { BottomSheetScrollView, SegmentedControl } from './PlatformControls';
...
export const OptionSheet: React.FC = () => {
  ...
  // Desktop sheets unmount when closed; restore the player's current selection.
  useEffect(() => {
    let mounted = true;
    TrackPlayer.getRepeatMode()
      .then((mode) => {
        if (mounted) setSelectedRepeatMode(repeatModeToIndex(mode));
      })
      .catch((error) => console.warn('Reading repeat mode failed', error));
    return () => {
      mounted = false;
    };
  }, []);
