...
Set a positive `progressUpdateEventInterval` with `TrackPlayer.updateOptions`.
The deadline is checked on the next progress event while playback is running,
not by an exact operating-system alarm. Check it again before resuming playback
after a pause or restart. This approach cannot wake a terminated app and follows
the same background-execution limits as progress tracking.
