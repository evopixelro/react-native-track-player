...
* Render the button as a spinner if `playWhenReady && (state === State.Loading || state === State.Buffering)`
* Else render the button as being in the Playing state if `playWhenReady && !(state === State.Error || state === State.Ended || state === State.None)`
