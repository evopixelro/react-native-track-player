...
Stops playback and releases the loaded media rather than simply pausing it.
Use `pause()` if you intend to resume the same loaded track.

On Web, `stop()` clears the active track index but retains the queue and the
previous `playWhenReady` setting. Select a track again before resuming; set
`playWhenReady` explicitly if the next selection must remain paused.
