...
| insertBeforeIndex | `number` | The index of the track that will be located immediately after the inserted tracks. Omit it or use `-1` to append to the queue. |
...
On Web, moving the active track or moving another track into its index is not implemented.
...
Updates the metadata displayed for the current item without replacing its queue
entry. Accepts track metadata fields and an optional `elapsedTime` in seconds;
the fields shown by system controls depend on the platform.
This method is not implemented on Web.
...
**Returns:** `Promise<`[Repeat Mode](../constants/repeat-mode.md)`>`
