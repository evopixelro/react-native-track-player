...
```ts
import { Event } from '@evopixel/react-native-track-player';
```

Availability differs by platform; see [Platform Support](../basics/platform-support.md).
...
Web reports playback failures through the `error` field on `PlaybackState`
instead of emitting this separate event.
...
### `RemotePlayPause` (Android and macOS)
Fired on Android for [`KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE`](https://developer.android.com/reference/android/view/KeyEvent#KEYCODE_MEDIA_PLAY_PAUSE), and on macOS for the system toggle command when Play and Pause capabilities are enabled.
...
### `RemoteLike`
Fired by the macOS Now Playing like command when `likeOptions` is enabled in `updateOptions`.

### `RemoteDislike`
Fired by the macOS Now Playing dislike command when `dislikeOptions` is enabled in `updateOptions`.

### `RemoteBookmark`
Fired by the macOS Now Playing bookmark command when `bookmarkOptions` is enabled in `updateOptions`.

These feedback event types remain available on iOS, but the 5.0.4 iOS capability
mapping does not enable their commands. See [Platform Support](../basics/platform-support.md).
...
Received data will be [`AudioCommonMetadataReceivedEvent`](./objects/metadata.md).
...
Received data will be [`AudioMetadataReceivedEvent`](./objects/metadata.md).

### `MetadataChapterReceived` (iOS only)
