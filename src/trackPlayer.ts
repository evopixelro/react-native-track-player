// Modified by EvoPixel to resolve bundled assets when replacing the queue
...
export function addEventListener<T extends Event>(
  ...
  // Keep the public event payload typed at React Native's variadic emitter boundary.
  return emitter.addListener(
    event,
    listener as Parameters<typeof emitter.addListener>[1]
  );
}
